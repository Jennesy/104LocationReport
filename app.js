...
function formHandler(event) {
	...
	if (event.target.id === 'form1') {
		...
		form1Btn.style.top = '0'
		form2Btn.style.top = '0.5rem'
	...
	form2Btn.style.top = '0'
	form1Btn.style.top = '0.5rem'
...
function getResults(target) {
	...
	const reportTime = `${target[2].value.padStart(
		2,
		'0'
	)}:${target[3].value.padStart(2, '0')}`
	...
	let transportation = target[4].checked
		? '步行'
	...
	const backTime = target[4].checked
		? '（已在家）'
		: `${target[23].value.padStart(2, '0')}:${target[24].value.padStart(
				2,
				'0'
		  )}`
	result.push(backTime)
	if (document.documentElement.dataset.theme === 'form2') {
		const endingTime = `${target[25].value.padStart(
			2,
			'0'
		)}:${target[26].value.padStart(2, '0')}`
		result.push(endingTime)
		let returnTransportation = document.querySelector(
			'input[name="return-transportation"]:checked'
		).value
		returnTransportation =
			returnTransportation === '其他' ? target[35].value : returnTransportation
		result.push(returnTransportation)
		const arriveTime = `${target[36].value.padStart(
			2,
			'0'
		)}:${target[37].value.padStart(2, '0')}`
